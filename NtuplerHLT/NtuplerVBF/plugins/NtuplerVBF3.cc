...
// Class:      NtuplerVBF3
// 
/**\class NtuplerVBF3 NtuplerVBF3.cc RecoBTag/NtuplerVBF3/src/NtuplerVBF3.cc
...
// $Id: NtuplerVBF3.cc,v 1.3 2012/02/02 09:04:04 arizzi Exp $
//
//
...
class NtuplerVBF3 : public edm::EDProducer {
	public:
		explicit NtuplerVBF3(const edm::ParameterSet&);

		int nevent,nrun,nlumi;

  TLorentzVector Higgs, b1, b2, q1, q2;
  float mqq, mbb, deltaetaqq, deltaetabb;
  int min, min1;
  float temp, temp2, temp3, temp4;
  float deltaphibb, deltaphiqq;

  float genParticlePt[4], genParticleEta[4];
  float genParticlePt_pTord[4],  genParticleEta_pTord[4];
  float genParticlePt_Etaord[4], genParticleEta_Etaord[4];
...
NtuplerVBF3::NtuplerVBF3(const edm::ParameterSet& iConfig)
{
  ...
  tree->Branch("mqq", &mqq, "mqq/F");
  tree->Branch("mbb", &mbb, "mbb/F");
  tree->Branch("deltaphiqq", &deltaphiqq, "deltaphiqq/F");
  tree->Branch("deltaphibb", &deltaphibb, "deltaphibb/F");
  tree->Branch("genParticlePt", &genParticlePt, "genParticlePt[4]/F");
  tree->Branch("genParticleEta",&genParticleEta ,"genParticleEta[4]/F");
  tree->Branch("genParticlePt_pTord", &genParticlePt_pTord, "genParticlePt_pTord[4]/F");
  tree->Branch("genParticleEta_pTord", &genParticleEta_pTord, "genParticleEta_pTord[4]/F");
  tree->Branch("genParticlePt_Etaord", &genParticlePt_Etaord, "genParticlePt_Etaord[4]/F");
  tree->Branch("genParticleEta_Etaord", &genParticleEta_Etaord, "genParticleEta_Etaord[4]/F");
	...
	void
NtuplerVBF3::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
...
mqq_CSVPFIDOrd=0;deltaetaqq_CSVPFIDOrd=0;deltaphibb_CSVPFIDOrd=0;deltaetabb_CSVPFIDOrd=0;ptsqq_CSVPFIDOrd=0;ptsbb_CSVPFIDOrd=0;signeta_CSVPFIDOrd=0;q1Eta_CSVPFIDOrd=0;q1Phi_CSVPFIDOrd=0;q1Pt_CSVPFIDOrd=0;q2Eta_CSVPFIDOrd=0;q2Phi_CSVPFIDOrd=0;q2Pt_CSVPFIDOrd=0;b1Eta_CSVPFIDOrd=0;b1Phi_CSVPFIDOrd=0;b1Pt_CSVPFIDOrd=0;b2Eta_CSVPFIDOrd=0;b2Phi_CSVPFIDOrd=0;b2Pt_CSVPFIDOrd=0;
  ...
  std::vector<const reco::Candidate*> genQuarks;
  ...
  for(GenParticleCollection::const_iterator g=genParticles->begin(); g!=genParticles->end(); ++g){
    if(g->pdgId()==25 && g->status()==3){
      Higgs.SetPtEtaPhiM(g->pt(), g->eta(),g->phi(),g->mass()); //Higgs TLorentzVector
      for(size_t i=0; i< g->numberOfDaughters(); ++i){
	const Candidate *hdaughter = g->daughter(i);
	if(hdaughter->pdgId()==5){  //it the boson decay product is a b quark
	  b1.SetPtEtaPhiM(hdaughter->pt(), hdaughter->eta(), hdaughter->phi(), hdaughter->mass());  
	  genQuarks.push_back(&*hdaughter);
	  printf("b_eta: %f \n", b1.Eta());
	}
	if(hdaughter->pdgId()==-5){  //it the boson decay product is an anti-b quark
	  b2.SetPtEtaPhiM(hdaughter->pt(), hdaughter->eta(), hdaughter->phi(), hdaughter->mass());  
	  genQuarks.push_back(&*hdaughter);
	}
      }
      const reco::Candidate *hmother1 = g->mother(0);  // higgs mother 1
      for(unsigned int i=0;i<hmother1->numberOfDaughters();++i){
	const reco::Candidate *pdaughter = hmother1->daughter(i);
	if(hmother1->numberOfDaughters()>2){
	  if (i>0 && i<3) genQuarks.push_back(&*pdaughter);
	  if (i==1) q1.SetPtEtaPhiM(pdaughter->pt(),pdaughter->eta(),pdaughter->phi(),pdaughter->mass());
	  if (i==2) q2.SetPtEtaPhiM(pdaughter->pt(),pdaughter->eta(),pdaughter->phi(),pdaughter->mass());
	}
      }
    }
  }
  ...
  mqq = (q1+q2).M();
  deltaetaqq = std::abs(q2.Eta()-q1.Eta());

  mbb=(b1+b2).M();
  deltaetabb = std::abs(b2.Eta()-b1.Eta());

  deltaphibb = deltaPhi(b1.Phi(), b2.Phi());
  deltaphiqq = deltaPhi(q1.Phi(), q2.Phi());
  
  genParticlePt[0]= b1.Pt();
  genParticlePt[1]= b2.Pt();
  genParticlePt[2]= q1.Pt();
  genParticlePt[3]= q2.Pt();
  
  genParticleEta[0]= b1.Eta();
  genParticleEta[1]= b2.Eta();
  genParticleEta[2]= q1.Eta();
  genParticleEta[3]= q2.Eta();
  
  //pT ordering
  for(int i=0; i<=3; i++){
    genParticlePt_pTord[i] = genParticlePt[i];
    genParticleEta_pTord[i]= genParticleEta[i];
  }
  
  for(int i=0; i<3; i++){
    min = i;	
    for(int j=i+1; j<4; j++){
      if(genParticlePt_pTord[j] > genParticlePt_pTord[min])
	min = j;
    }
    temp=genParticlePt_pTord[min];
    genParticlePt_pTord[min]=genParticlePt_pTord[i];
    genParticlePt_pTord[i]=temp;  

    temp2=genParticleEta_pTord[min];
    genParticleEta_pTord[min]=genParticleEta_pTord[i];
    genParticleEta_pTord[i]=temp2;
  }

  //eta ordering

  for(int i=0; i<=3; i++){
    genParticlePt_Etaord[i] = genParticlePt[i];
    genParticleEta_Etaord[i]= genParticleEta[i];
  }
  
  for(int i=0; i<3; i++){
    min1 = i;	
    for(int j=i+1; j<4; j++){
      if(genParticleEta_Etaord[j] > genParticleEta_Etaord[min1])
	min1 = j;
    }
    temp3=genParticlePt_Etaord[min1];
    genParticlePt_Etaord[min1]=genParticlePt_Etaord[i];
    genParticlePt_Etaord[i]=temp3;  

    temp4=genParticleEta_Etaord[min1];
    genParticleEta_Etaord[min1]=genParticleEta_Etaord[i];
    genParticleEta_Etaord[i]=temp4;
  }
	...
	//Eta ordered with Calo jets
	{
		...
		if(!CalojH.failedToGet())
		{
			...
			if(nJet>=4)
			{
				...
				q1deltaR_etaOrd	=	std::min(deltaR(q1.eta(),q1.phi(),q1Eta_gen,q1Phi_gen),deltaR(q1.eta(),q1.phi(),q2Eta_gen,q2Phi_gen));
				q2deltaR_etaOrd	=	std::min(deltaR(q2.eta(),q2.phi(),q1Eta_gen,q1Phi_gen),deltaR(q2.eta(),q2.phi(),q2Eta_gen,q2Phi_gen));
				b1deltaR_etaOrd	=	std::min(deltaR(b1.eta(),b1.phi(),b1Eta_gen,b1Phi_gen),deltaR(b1.eta(),b1.phi(),b2Eta_gen,b2Phi_gen));
				b2deltaR_etaOrd	=	std::min(deltaR(b2.eta(),b2.phi(),b1Eta_gen,b1Phi_gen),deltaR(b2.eta(),b2.phi(),b2Eta_gen,b2Phi_gen));
			}
		}	
	}
	...
	//Eta ordered with Calo jets no PU
	{
		...
		if(!CalojH.failedToGet())
		{
			...
			if(nJet>=4)
			{
				...
				q1deltaR_etaOrdNoPU	=	std::min(deltaR(q1.eta(),q1.phi(),q1Eta_gen,q1Phi_gen),deltaR(q1.eta(),q1.phi(),q2Eta_gen,q2Phi_gen));
				q2deltaR_etaOrdNoPU	=	std::min(deltaR(q2.eta(),q2.phi(),q1Eta_gen,q1Phi_gen),deltaR(q2.eta(),q2.phi(),q2Eta_gen,q2Phi_gen));
				b1deltaR_etaOrdNoPU	=	std::min(deltaR(b1.eta(),b1.phi(),b1Eta_gen,b1Phi_gen),deltaR(b1.eta(),b1.phi(),b2Eta_gen,b2Phi_gen));
				b2deltaR_etaOrdNoPU	=	std::min(deltaR(b2.eta(),b2.phi(),b1Eta_gen,b1Phi_gen),deltaR(b2.eta(),b2.phi(),b2Eta_gen,b2Phi_gen));
			}
		}	
	}
	...
	//Eta ordered with PF jets
	{
		...
		if(!PFjH.failedToGet())
		{
			...
			if(nJet>=4)
			{
				...
				q1deltaR_etaPFOrd	=	std::min(deltaR(q1.eta(),q1.phi(),q1Eta_gen,q1Phi_gen),deltaR(q1.eta(),q1.phi(),q2Eta_gen,q2Phi_gen));
				q2deltaR_etaPFOrd	=	std::min(deltaR(q2.eta(),q2.phi(),q1Eta_gen,q1Phi_gen),deltaR(q2.eta(),q2.phi(),q2Eta_gen,q2Phi_gen));
				b1deltaR_etaPFOrd	=	std::min(deltaR(b1.eta(),b1.phi(),b1Eta_gen,b1Phi_gen),deltaR(b1.eta(),b1.phi(),b2Eta_gen,b2Phi_gen));
				b2deltaR_etaPFOrd	=	std::min(deltaR(b2.eta(),b2.phi(),b1Eta_gen,b1Phi_gen),deltaR(b2.eta(),b2.phi(),b2Eta_gen,b2Phi_gen));
			}
		}	
	}
	...
	//Eta ordered with PF jets with JetID
	{
		...
		if(!PFjH.failedToGet())
		{
			...
			if(nJet>=4)
			{
				...
				q1deltaR_etaPFIDOrd	=	std::min(deltaR(q1.eta(),q1.phi(),q1Eta_gen,q1Phi_gen),deltaR(q1.eta(),q1.phi(),q2Eta_gen,q2Phi_gen));
				q2deltaR_etaPFIDOrd	=	std::min(deltaR(q2.eta(),q2.phi(),q1Eta_gen,q1Phi_gen),deltaR(q2.eta(),q2.phi(),q2Eta_gen,q2Phi_gen));
				b1deltaR_etaPFIDOrd	=	std::min(deltaR(b1.eta(),b1.phi(),b1Eta_gen,b1Phi_gen),deltaR(b1.eta(),b1.phi(),b2Eta_gen,b2Phi_gen));
				b2deltaR_etaPFIDOrd	=	std::min(deltaR(b2.eta(),b2.phi(),b1Eta_gen,b1Phi_gen),deltaR(b2.eta(),b2.phi(),b2Eta_gen,b2Phi_gen));
			}
		}	
	}
	...
	//Eta ordered with trackJet
	{
		...
		if(!TrackjH.failedToGet())
		{
			...
			if(nJet>=4)
			{
				...
				q1deltaR_etaTrackOrd	=	std::min(deltaR(q1.eta(),q1.phi(),q1Eta_gen,q1Phi_gen),deltaR(q1.eta(),q1.phi(),q2Eta_gen,q2Phi_gen));
				q2deltaR_etaTrackOrd	=	std::min(deltaR(q2.eta(),q2.phi(),q1Eta_gen,q1Phi_gen),deltaR(q2.eta(),q2.phi(),q2Eta_gen,q2Phi_gen));
				b1deltaR_etaTrackOrd	=	std::min(deltaR(b1.eta(),b1.phi(),b1Eta_gen,b1Phi_gen),deltaR(b1.eta(),b1.phi(),b2Eta_gen,b2Phi_gen));
				b2deltaR_etaTrackOrd	=	std::min(deltaR(b2.eta(),b2.phi(),b1Eta_gen,b1Phi_gen),deltaR(b2.eta(),b2.phi(),b2Eta_gen,b2Phi_gen));
			}
		}	
	}
	...
	//CSVL25 ordered 
	{
		...
		if(nJet>=4) {
			...
			q1deltaR_CSVL25Ord	=	std::min(deltaR(q1.eta(),q1.phi(),q1Eta_gen,q1Phi_gen),deltaR(q1.eta(),q1.phi(),q2Eta_gen,q2Phi_gen));  	
			q2deltaR_CSVL25Ord	=	std::min(deltaR(q2.eta(),q2.phi(),q1Eta_gen,q1Phi_gen),deltaR(q2.eta(),q2.phi(),q2Eta_gen,q2Phi_gen));  	
			b1deltaR_CSVL25Ord	=	std::min(deltaR(b1.eta(),b1.phi(),b1Eta_gen,b1Phi_gen),deltaR(b1.eta(),b1.phi(),b2Eta_gen,b2Phi_gen));  	
			b2deltaR_CSVL25Ord	=	std::min(deltaR(b2.eta(),b2.phi(),b1Eta_gen,b1Phi_gen),deltaR(b2.eta(),b2.phi(),b2Eta_gen,b2Phi_gen)); 
		}
	}
...
//		const JetTagCollection& jetTagss = *jetTags;
		...
		if(nJet>=4) {
			...
			q1deltaR_CSVL3Ord	=	std::min(deltaR(q1.eta(),q1.phi(),q1Eta_gen,q1Phi_gen),deltaR(q1.eta(),q1.phi(),q2Eta_gen,q2Phi_gen));  	
			q2deltaR_CSVL3Ord	=	std::min(deltaR(q2.eta(),q2.phi(),q1Eta_gen,q1Phi_gen),deltaR(q2.eta(),q2.phi(),q2Eta_gen,q2Phi_gen));  	
			b1deltaR_CSVL3Ord	=	std::min(deltaR(b1.eta(),b1.phi(),b1Eta_gen,b1Phi_gen),deltaR(b1.eta(),b1.phi(),b2Eta_gen,b2Phi_gen));  	
			b2deltaR_CSVL3Ord	=	std::min(deltaR(b2.eta(),b2.phi(),b1Eta_gen,b1Phi_gen),deltaR(b2.eta(),b2.phi(),b2Eta_gen,b2Phi_gen)); 
		}
	}
		...
		//CSVPF ordered 
	{
		...
		if(nJet>=4) {
			...
			q1deltaR_CSVPFOrd	=	std::min(deltaR(q1.eta(),q1.phi(),q1Eta_gen,q1Phi_gen),deltaR(q1.eta(),q1.phi(),q2Eta_gen,q2Phi_gen));  	
			q2deltaR_CSVPFOrd	=	std::min(deltaR(q2.eta(),q2.phi(),q1Eta_gen,q1Phi_gen),deltaR(q2.eta(),q2.phi(),q2Eta_gen,q2Phi_gen));  	
			b1deltaR_CSVPFOrd	=	std::min(deltaR(b1.eta(),b1.phi(),b1Eta_gen,b1Phi_gen),deltaR(b1.eta(),b1.phi(),b2Eta_gen,b2Phi_gen));  	
			b2deltaR_CSVPFOrd	=	std::min(deltaR(b2.eta(),b2.phi(),b1Eta_gen,b1Phi_gen),deltaR(b2.eta(),b2.phi(),b2Eta_gen,b2Phi_gen)); 
		}
	}
...
//			q1deltaR_CSVPFIDOrd	=	std::min(deltaR(q1.eta(),q1.phi(),q1Eta_gen,q1Phi_gen),deltaR(q1.eta(),q1.phi(),q2Eta_gen,q2Phi_gen));  	
//			q2deltaR_CSVPFIDOrd	=	std::min(deltaR(q2.eta(),q2.phi(),q1Eta_gen,q1Phi_gen),deltaR(q2.eta(),q2.phi(),q2Eta_gen,q2Phi_gen));  	
//			b1deltaR_CSVPFIDOrd	=	std::min(deltaR(b1.eta(),b1.phi(),b1Eta_gen,b1Phi_gen),deltaR(b1.eta(),b1.phi(),b2Eta_gen,b2Phi_gen));  	
//			b2deltaR_CSVPFIDOrd	=	std::min(deltaR(b2.eta(),b2.phi(),b1Eta_gen,b1Phi_gen),deltaR(b2.eta(),b2.phi(),b2Eta_gen,b2Phi_gen)); 
//		}
	}
