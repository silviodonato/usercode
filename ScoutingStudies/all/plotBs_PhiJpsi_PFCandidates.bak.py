...
for inputFile in inputFiles:
    ...
    #events.SetBranchStatus("*pfCand*g4SimHits*", 1)
    ...
    pfCands, pfCandLabel = Handle("vector<Run3ScoutingParticle>"), ("hltScoutingPFPacker","","HLTX")
...
#    genParticles, genParticleLabel = Handle("vector<reco::GenParticle>"), ("genParticles","","HLT")
    ...
    histoScouting2mu = ROOT.TH1F("histoScouting2mu","",100,5,6)
...
#                if pfCand.momentum().pt()>2.5 and pfCand.charge()!=0:
#                    pt = pfCand.momentum().pt()
#                    eta = pfCand.momentum().eta()
#                    phi = pfCand.momentum().phi()
#                    mass = max(1E-9, pfCand.momentum().mass())
#        #            mass = 0.14
...
#        momenta = [ (track.pt(), track.eta(), track.phi(), pion_mass) for track in pfCands.product() if abs(track.pdgId())==211 and track.pt()>2]
        momenta = [ (track.pt() + track.trk_pt(), track.eta() + track.trk_eta(), track.phi() + track.trk_phi(), pion_mass) for track in pfCands.product() if abs(track.pdgId())==211 and track.pt()>3 and track.vertex()==0]
#        momenta = [ (track.tk_pt(), track.tk_eta(), track.tk_phi(), pion_mass) for track in pfCands.product()]
        
        mass = -1
        n_mu = -1
        ## search j/psi peak between 2 muons
        if len(momenta_mu)>=2 and len(momenta)>=2:
            mu_1, mu_2 = findBestPair(momenta_mu, phi_mass, muon_mass, muon_mass)
        ...
        if n_mu < 0 and len(momenta_mu)>=1 and len(momenta)>=3:
            mu_1, mu_2 = findBestPairTwoCollections(momenta_mu, momenta, phi_mass, muon_mass, muon_mass)
        ...
        if n_mu < 0 and len(momenta)>=4 :
            mu_1, mu_2 = findBestPair(momenta, phi_mass, muon_mass, muon_mass)
...
#                mass = 0.14
                ...
                if matching(0.03, mu1[1], track[1], mu1[2], track[2]) and not mu1_trk: 
                    momenta.remove(track)
                    mu1_trk = track
                elif matching(0.03, mu2[1], track[1], mu2[2], track[2]) and not mu2_trk: 
                    momenta.remove(track)
                    mu2_trk = track
...
#        masses.append((phi1+phi2).M())
    ...
    #        print(masses)
        ...
        if abs(phi_mass - (phi1+phi2).M())>0.03: continue
...
#        for mass in masses: histoScouting.Fill(mass)
    ...
    #        if pfCand.momentum().pt()>2 and pfCand.charge()!=0:
    #            print(pfCand.momentum().pt(), pfCand.type(), pfCand.trackId(), pfCand.genpartIndex())
    #            for pfCand2 in pfCands.product():
    #                if pfCand2.momentum().pt()>2 and pfCand2.charge()!=0 and pfCand2!=pfCand:
    #                    for pfCand3 in pfCands.product():
    #                        if pfCand3.momentum().pt()>2 and pfCand3.charge()!=0 and pfCand3!=pfCand and pfCand3!=pfCand2:
    #                            for pfCand4 in pfCands.product():
    #                                if pfCand4.momentum().pt()>2 and pfCand4.charge()!=0 and pfCand4!=pfCand and pfCand4!=pfCand2 and pfCand4!=pfCand3:
    #                                    histo.Fill((pfCand.momentum()+pfCand2.momentum()+pfCand3.momentum()+pfCand4.momentum()).M())
    #                    histo.Fill(invariant_mass(pfCand.momentum().pt(), pfCand2.momentum().pt(), pfCand.momentum().eta(), pfCand2.momentum().eta(), pfCand.momentum().phi(), pfCand2.momentum().phi(), max(1E-3, pfCand.momentum().mass()), max(1E-3, pfCand2.momentum().mass())))
    #                for pfCand2 in pfCands.product():
    #                    if pfCand2.momentum().pt()>2 and pfCand2.charge()!=0:
    #                    histo.Fill(invariant_mass(pfCand.momentum().pt(), pfCand2.momentum().pt(), pfCand.momentum().eta(), pfCand2.momentum().eta(), pfCand.momentum().phi(), pfCand2.momentum().phi(), 0, 1E-3))
    ...
    fName = inputFile
    fName = fName.replace("output","plotPFCands_")
    histo.Draw()
    c1.SaveAs(fName)
...
#    f1.SetRange(m-1.75*s, m+1.75*s)
    ...
    print("Integral: m, s: %f %f"%(f2.GetParameter(1), f2.GetParameter(2)))
