...
#        momenta = [ (track.pt() + track.trk_pt(), track.eta() + track.trk_eta(), track.phi() + track.trk_phi(), pion_mass) for track in pfCands.product() if track.normchi2()!=999 and track.pt()>3]
        momenta = [ (track.pt(), track.eta(), track.phi(), pion_mass) for track in pfCands.product() if track.normchi2()!=999 and track.pt()>2]
...
#        for mass in masses: histoScouting.Fill(mass)
    ...
    fName = inputFile
    fName = fName.replace("output","plot2GeVPFCand")
#    histo.Draw()
#    c1.SaveAs(fName)
...
#    f1.SetRange(m-1.75*s, m+1.75*s)
    ...
    f2.SetParameters(f1.GetParameter(0), f1.GetParameter(1), f1.GetParameter(2)*1.5, 30, 5.5, 1)
    ## remove second gaussian
    f2.FixParameter(3,0)
    f2.FixParameter(4,0)
    f2.FixParameter(5,0)
