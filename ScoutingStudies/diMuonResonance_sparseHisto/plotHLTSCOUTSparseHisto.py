...
print("Entries:", sparseHisto.GetEntries())
print("Dimensions:", sparseHisto.GetNdimensions())
...
rebin = 10
selectionJpsi = {
    "mass_mumu" : [massJpsi-0.1, massJpsi+0.1],
    # "dxy" : [-999, 0.9],
}
...
plotDefs = { ## plot name: (variable , selection)
    "dxy_both" : ( "dxy" , selection, rebin),
    "dxyJpsi_both" : ( "dxy" , selectionJpsi, rebin),
    "dxySig_both" : ( "dxySig" , selection, rebin),
    "dxySigJpsi_both" : ( "dxySig" , selectionJpsi, rebin),
    
    "mass_mumu_both" : ( "mass_mumu" , selection, rebin),
    "mass_mumu_full_both" : ( "mass_mumu" , selection, rebin*2, [-999, 5]),
    "pt_both" : ( "pt" , selection, rebin, [-999, 9999]),
    # "pt_eff_DST_PFScouting_DoubleMuon" : ( "pt" , selection, rebin, [-999, 9999]),
    "p2_eff_DST_PFScouting_DoubleMuon" : ( "ptmin" , selection, rebin, [0, 999]),
    "p2Jpsi_eff_DST_PFScouting_DoubleMuon" : ( "ptmin" , selectionJpsi, rebin, [0, 999]),
    "p1_eff_DST_PFScouting_DoubleMuon" : ( "ptmax" , selection, rebin, [0, 999]),
    "p1Jpsi_eff_DST_PFScouting_DoubleMuon" : ( "ptmax" , selectionJpsi, rebin, [0, 999]),
    # "p2_inclusive_eff_DST_PFScouting_DoubleMuon" : ( "ptmin" , {}, rebin, [0, 999]),
    # "pt2_eff_DST_PFScouting_SingleMuon" : ( "ptmin" , selection, rebin, [0, 999]),
    # "pt1_eff_DST_PFScouting_ZeroBias" : ( "ptmin" , selection, rebin, [0, 999]),
    # "pt2_both" : ( "ptmin" , selection, rebin, [-10, 6000]),
    # "pt1_both" : ( "ptmax" , selection, rebin, [-10, 6000]),
    # "dxy_both" : ( "dxy" ,selection,rebin, [-999, 9999]),
    # "dxySig_both" : ( "dxySig" ,selection,rebin , [-999, 9999]),
    # "deltaR_both" : ( "deltaR" ,selection,rebin, [0.00,1]),
    # "deltaPhi_both" : ( "deltaPhi" ,selection,rebin, [0.00,1]),
    # "deltaEta_both" : ( "deltaEta" ,selection,rebin, [0.00,1]),
...
def makePlot(sparseHisto, plotDef):
    ...
    histo.GetXaxis().SetRangeUser(xmin-binWidth, xmax+binWidth)
    return histo
...
for plotName in plotDefs:
    plotDef = deepcopy(plotDefs[plotName])
    if "_both" in plotName: ## plot opposite sign vs same sign
        for ssos in ["os", "ss"]:
        ...
        leg.Draw()
    elif "_eff_" in plotName: ## turn on efficiency
        plot, trigger = plotName.split("_eff_")
        print("DENOMINATOR")
        den = makePlot(sparseHisto, plotDef)
        den.SetName(plotName+"_den")
        plotDefMod = deepcopy(plotDef)
        plotDefMod[1].update({trigger : [1, 9999]})
        print("NUMERATOR")
        num = makePlot(sparseHisto, plotDefMod)
        num.SetName(plotName+"_num")
        den.Draw()
        num.SetLineColor(ROOT.kRed)
        num.Draw("same")
        canvas.SaveAs("%s/%s_den.png"%(plotFolder,plotName))
        # eff = num.Clone()
        eff = ROOT.TEfficiency(num, den)
        # eff.Divide(den, num, 1., 1., "B")
        ...
        # eff.GetXaxis().SetTitle(plotDef[0])
