...
For data after CMSHLT- (April 2024), you need to run the L1 unpacker manually (eg. cmsRun unpackL1FEDRawToGtStage2Digis.py file:/eos/cms/tier0/store/data/Run2024G/L1Accept/RAW/v1/000/383/948/00000/a43106b3-57a9-4be8-8d26-ee56a1b506a7.root)

This code will print the L1 trigger bit before PS, afert mask, after prescale, and allows you to apply a selection based on HLT.
...
if len(sys.argv)<2:
    raise Exception("Two args required. Eg. python3 -i printTriggerBit.py gtStage2Digis.root")
...
DST_ZeroBias = names.triggerIndex("DST_ZeroBias_v20")
DST_Physics = names.triggerIndex("DST_Physics_v15")
HLT_ZeroBias = names.triggerIndex("HLT_ZeroBias_v12")
count = 0
for iev,event in enumerate(events):
    if iev>5000: break
    #if event.eventAuxiliary().luminosityBlock()!=34: continue
    val = event.getByLabel(triggerBitLabel, triggerBits)
    if triggerBits.product().accept(DST_Physics): ##and riggerBits.product().accept(HLT_ZeroBias)
        count+=1
        lumi = event.eventAuxiliary().luminosityBlock()
