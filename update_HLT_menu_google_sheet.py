...
credentials = ServiceAccstreams = "GRun.csv" # hltGetConfiguration /dev/CMSSW_13_0_0/GRun/V16 > hlt.py && cat hlt.py | hltDumpStream --mode csv > GRun.csv
refLumi = 2.4E34
maxPath = 10000
...
print()streams = "GRun.csv" # hltGetConfiguration /dev/CMSSW_13_0_0/GRun/V16 > hlt.py && cat hlt.py | hltDumpStream --mode csv > GRun.csv
refLumi = 2.4E34
maxPath = 10000
...
ountCredentials.from_json_keyfile_name(api_json_credentials, scope)
gc = gspread.authorize(credentials)
...
purerates_paths = getRatesFromSpreadsheet(spreadsheetId='1Cr60H1Q1PJov2ADo0Q3kVqxvXhr8NEInGgTVxV5c0ec', worksheetId=551224544, labelPath="Path (w/ version number)", labelColumns=["Pure Rate (Hz)"])
rates_paths = getRatesFromSpreadsheet(spreadsheetId='1Cr60H1Q1PJov2ADo0Q3kVqxvXhr8NEInGgTVxV5c0ec', worksheetId=1154525798, labelPath="Path (w/ version number)", labelColumns=["PU64"])

#for trigger in rates_paths:
#    pureRate = ""
#    if trigger in purerates_paths: pureRate = purerates_paths[trigger]
#    rates_paths[trigger].append(pureRate)


#OMS_rates_2018 = getRates2018(rates2018)
streams_paths = getStreams(streams)
OMS_rates_2018 = getOMSRates(list(streams_paths)[:maxPath], run=322079, minLS=39, maxLS=42, removeDeadtime=True, scale=refLumi/2.055E34*0.96)  ##PU57.49 2.055E34 4% deadtime
OMS_rates_2022 = getOMSRates(list(streams_paths)[:maxPath], run=362616, minLS=302, maxLS=306, removeDeadtime=True, scale=refLumi/21.19E33*0.96) ##PU61.5 21.19E33 4% deadtime
#rates_paths = OMS_rates_2022
#old_paths = getMenu(oldFile)
#old_paths = getCurrentMenu()

import pprint
#pprint.pprint(old_paths[0])
pprint.pprint(rates_paths["HLT_PFJet200_v"])
pprint.pprint(streams_paths["HLT_PFJet200_v"])
...
def updatedLine(headers, line, path, streams_paths, rates_paths, OMS_rates_2018, OMS_rates_2022):
    newLine = line[:]
    for i, column in enumerate(headers):
        if column == "PS":
            newLine[i] = streams_paths[path][" 2p0E34+ZeroBias+HLTPhysics"]
            ...
            newLine[i]  = rates_paths[path][0] if path in rates_paths else ""
        elif column == "Pure Rate":
            newLine[i]  = purerates_paths[path][0] if path in purerates_paths else ""
#        elif column == "Owners": ## keep the original formula in Owners
#            newLine[i]  = ''
        elif column == "path":
            newLine[i]  = path
...
for i, line in enumerate(ws[:]):
    ...
    if path in streams_paths:
        new_paths.remove(path)
#        print(ws[i])
#        print(updatedLine(menu_header, ws[i], path, streams_paths, rates_paths, OMS_rates_2018, OMS_rates_2022))
        ws[i] = updatedLine(menu_header, ws[i], path, streams_paths, rates_paths, OMS_rates_2018, OMS_rates_2022)
    else:
        ws[i] = ['DELETED '+path] + ['']*10
        deletedPaths.append(path)

for path in new_paths:
    ws.append(updatedLine(menu_header, [""]*len(menu_header), path, streams_paths, rates_paths, OMS_rates_2018, OMS_rates_2022))
